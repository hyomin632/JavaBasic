...
public class HelloJDBC {
    public static void main(String[] args) {
        ...
        // 이것을 통해 데이터베이스 코드를 한 번만 작성해두면 어떤 DBMS라도 코드 변경 없이
        // 동일하게 작동할 수 있음

        // JDK에 포함된 JDBC인터페이스는 java.sql 패키지 아래에 있음
        // 하지만 이것만으로는 작동하지 않고, 각 데이터베이스 제조사가 JDBC인터페이스를 구현해서
        ...
        String ourl = "jdbc:oracle:thin:@192.168.116.3:1521:xe";
        String ousr = "system";
        ...
        String purl = "jdbc:postgresql://192.168.116.3:5432/postgres";
        ...
        try (
                Connection conn = DriverManager.getConnection(purl, pusr, pwd);
        ) {
            if (conn != null)
                System.out.println("데이터베이스 연결 성공!");
        } catch (Exception ex) {
