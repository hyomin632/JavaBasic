...
public class JDBCCRUD {
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        // 새로운 도서 정보를 입력하는 프로그램을 작성
        // 컬럼 정보: bookid, bookname, publisher, price
        // 입력 방법은 Scanner 클래스 이용

        // JDBC 접속 정보
        String drv = "org.mariadb.jdbc.Driver";
        String amz = "bigdata.ce67r0x8wwmd.ap-northeast-2.rds.amazonaws.com";
        String url = "jdbc:mariadb://"+amz+":3306/playground";
        String usr = "playground";
        String pwd = "bigdata2020";

        // JDBC 관련 sql 정의
        String books = " create table books (bookid int, bookname varchar(20), " +
                       " publisher varchar(20), price int) ";
        String newbooksql = " insert into books values (1, 'java', 'oracle', 10000) ";
        String newbooksql2 = " insert into books values ('%d','%s','%s','%d') ";

        // JDBC 작업 시작
        Class.forName(drv); // JDBC 드라이버 메모리에 적재
        try (Connection conn = DriverManager.getConnection(url, usr, pwd);
             // 드라이버 매너저를 이용해서 DBMS에 접속하고, 세션을 하나 가져옴
