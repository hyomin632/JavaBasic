...
public class Methods {
    public static void main(String[] args) {
        // 메서드: 특정 작업을 수행하기 위한 코드들의 집합. c언어의 함수와 유사
        // 유사한 코드들의 반복적인 사용을 피할 수 있음
        // 모듈화를 통해 프로그램의 이해도 증가
        // 코드의 문제 발생 또는 기능 변경시에 손쉽게 유지 보수 가능

        // 메서드 형식: 접근 지정자(제한자) 반환 타입 메서드명(매개변수 목록) {
        //                   메서드 몸체
        //                   return 변수
        //              }
        // 접근 지정자: 메서드 접근 허용 범위 지정(public/private)
        // 반환 타입: 메서드 실행 후 반환(return)하는 자료의 타입. 반환 타입이 없는 경우 void로 정의
        // 매개변수 목록: 메서드 호출시 전달하는 변수 지정

        // 메서드 호출: 메서드명() / 메서드명(매개변수값)
